...
from nemo_rl.algorithms.dapo import MasterConfig, dapo_train, setup
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(description="Run DAPO training with configuration")
    parser.add_argument(
...
def main() -> None:
    ...
    if not args.config:
        args.config = os.path.join(
            os.path.dirname(__file__), "configs", "dapo_math_1B.yaml"
        )
    ...
    assert config["policy"]["generation"] is not None, (
        "A generation config is required for DAPO"
    )
    ...
    (
        ...
        task_to_env,
        val_task_to_env,
    ) = setup_data(tokenizer, config["data"], config["env"], config["dapo"]["seed"])
