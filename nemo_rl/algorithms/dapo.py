...
def _default_dapo_save_state() -> DAPOSaveState:
    return {
        "step": 0,
...
class DAPOLoggerConfig(LoggerConfig):
...
def setup(
    ...
    """Main entry point for running DAPO algorithm.
    ...
    dapo_config = master_config["dapo"]
    data_config = master_config["data"]
    logger_config = master_config["logger"]
    cluster_config = master_config["cluster"]

    assert generation_config is not None, (
        "A generation config in the PolicyConfig is required for DAPO"
    )

    # Set seed for all random number generators
    set_seed(dapo_config["seed"])
    ...
    dapo_save_state: Optional[DAPOSaveState] = cast(
        Optional[DAPOSaveState], checkpointer.load_training_info(last_checkpoint_path)
    )
    if dapo_save_state is None:
        dapo_save_state = _default_dapo_save_state()
    ...
    dataloader = StatefulDataLoader(
        dataset,
        batch_size=dapo_config["num_prompts_per_step"],
    ...
    if dapo_config["val_period"] > 0 or dapo_config["val_at_start"]:
        assert val_dataset is not None, (
            "Validation dataset is required if validation is enabled"
        )
        val_dataloader = StatefulDataLoader(
            val_dataset,
            batch_size=dapo_config["val_batch_size"],
            shuffle=False,
            collate_fn=rl_collate_fn,
        )
...
def dapo_train(
    ...
    dapo_save_state: DAPOSaveState,
    master_config: MasterConfig,
) -> None:
    """Run DAPO training algorithm."""
    timer = Timer()
    ...
    step = dapo_save_state["step"]
    consumed_samples = dapo_save_state["consumed_samples"]
    val_period = master_config["dapo"]["val_period"]
    val_at_start = master_config["dapo"]["val_at_start"]
    ...
    # This is the number of batches we processed so far for each step to generate responses whose std is non-zero. Maximum threshold is set by max_num_gen_batches.
    num_gen_batches = 0

    batch: BatchedDataDict[DatumSpec] = None
    # Run dapo training loop
    for current_batch in dataloader:
        print(
            f"\n{'=' * 25} Step {step + 1}/{min(len(dataloader), master_config['dapo']['max_num_steps'])} {'=' * 25}"
        )
        ...
        with timer.time("total_step_time"):
            ...
            with timer.time("data_processing"):
                ...
                repeated_batch: BatchedDataDict[DatumSpec] = (
                    current_batch.repeat_interleave(
                        master_config["dapo"]["num_generations_per_prompt"]
                    )
                )
            ...
            with timer.time("generation"):
                ...
                if _should_use_async_rollouts(master_config):
                    (
                        ...
                        max_rollout_turns=master_config["dapo"]["max_rollout_turns"],
                        greedy=False,
                    )
                else:
            ...
            with timer.time("reward_calculation"):
                ...
                baseline, std = calculate_baseline_and_std_per_prompt(
                    ...
                    leave_one_out_baseline=master_config["dapo"][
                ...
                # If sampled prompts (with non-zero std) are fewer than train_batch_size * num_generations_per_prompt, continue sampling until max_num_gen_batches is reached.
                if master_config["dapo"]["use_dynamic_sampling"]:
                    ...
                    if repeated_batch.size > 0:
                        # Concatenate the previous half-filled batch with the current batch. This serves as a cache to store and collect the prompts with non-zero std.
                        # This is used in the next iteration when the current batch is not enough to fill the buffer.
                        batch = (
                            repeated_batch
                            if batch is None
                            else BatchedDataDict.from_batches([batch, repeated_batch])
                        )
                    ...
                    if generation_sample_buffer_size < train_prompts_buffer_size:
                        max_num_gen_batches = master_config["dapo"].get(
                        ...
                        if (
                            ...
                            print(
                                f"Generation sample buffer size: {generation_sample_buffer_size} is smaller than train_prompts_buffer_size: {train_prompts_buffer_size}. Processed {num_gen_batches} batches so far out of {max_num_gen_batches}."
                            )
                            continue
                        else:
                            raise ValueError(
                        ...
                        # Gather the corresponding baseline(mean) and std
                ...
                # Normalize rewards
                if master_config["dapo"]["normalize_rewards"]:
            ...
            is_last_step = step + 1 == min(
                master_config["dapo"]["max_num_steps"], len(dataloader)
            )
            ...
            ## Checkpointing
            consumed_samples += master_config["dapo"]["num_prompts_per_step"]
            ...
            if master_config["checkpointing"]["enabled"] and (
                ...
                dapo_save_state["step"] = step + 1
                if val_metrics is not None:
                    dapo_save_state["val_reward"] = val_metrics["accuracy"]
                elif "val_reward" in dapo_save_state:
                    del dapo_save_state["val_reward"]
                dapo_save_state["consumed_samples"] = consumed_samples

                if master_config["checkpointing"]["metric_name"] is not None:
                    if (
                        master_config["checkpointing"]["metric_name"]
                        not in dapo_save_state
                    ):
                        warnings.warn(
                ...
                with timer.time("checkpointing"):
                    ...
                    checkpoint_path = checkpointer.init_tmp_checkpoint(
                        step + 1, dapo_save_state, master_config
                    )
        ...
        timer.reset()
        step += 1
        if step >= master_config["dapo"]["max_num_steps"]:
            break
...
def validate(
    ...
    with timer.time("total_validation_time"):
        ...
        max_batches = (
            master_config["dapo"]["max_val_samples"]
            // master_config["dapo"]["val_batch_size"]
        )
