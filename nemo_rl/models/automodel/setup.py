...
def setup_model_and_optimizer(
    ...
    # computed default. Without the pop, the explicit kwarg below would clash
    # with **automodel_kwargs and from_pretrained() raises TypeError on
    # duplicate keyword.
    ...
    # Modality-tower freezing is now declarative in recipe YAML via the
    # `policy.dtensor_cfg.freeze_config` block (handled by Automodel's
    # apply_parameter_freezing). Frozen towers don't get optimizer state, so
    # this also avoids the ckpt-resume key mismatch that the old hardcoded
    # block was working around. Add freeze_config to text-only VLM recipes.
