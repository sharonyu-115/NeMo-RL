...
def kv_cache_process_weights_after_loading(self, layer: torch.nn.Module) -> None:
    ...
    # [DEBUG] Log layer scale values
    tp_rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    k_val = layer.k_scale.item() if hasattr(layer, 'k_scale') and isinstance(layer.k_scale, torch.Tensor) else getattr(layer, 'k_scale', 'N/A')
    v_val = layer.v_scale.item() if hasattr(layer, 'v_scale') and isinstance(layer.v_scale, torch.Tensor) else getattr(layer, 'v_scale', 'N/A')
    q_val = layer.q_scale.item() if hasattr(layer, 'q_scale') and isinstance(layer.q_scale, torch.Tensor) else getattr(layer, 'q_scale', 'N/A')
    print(f"[DEBUG TP rank {tp_rank}] kv_cache_process_weights_after_loading: k_scale={k_val}, v_scale={v_val}, q_scale={q_val}, kv_cache_dtype={layer.kv_cache_dtype}")
    ...
    if name not in fp8_state.seen_params:
        ...
        if name.endswith("weight"):
            ...
            if (
                ...
                print(f"[SHARON]Added layer {name} to fp8_state.fp8_param_names")
...
def load_weights(weights, model_runner):
    ...
    for k, v in weights:
        #print(f"[SHARON] in the load_weights function, check and print the value for k_scale and v_scale")
        # only print the value when k contains k_scale and v_scale
    ...
    print("[SHARON] After model.load_weights, collecting garbage and emptying cache")
    print(f"[SHARON] Before del weights_quantized, torch.cuda.memory_allocated() in GB: {torch.cuda.memory_allocated() / 1024 / 1024 / 1024}")
    del weights_quantized   
    torch.cuda.empty_cache()
    print(f"[SHARON] After del weights_quantized and torch.cuda.empty_cache(), torch.cuda.memory_allocated() in GB: {torch.cuda.memory_allocated() / 1024 / 1024 / 1024}")
