...
def _quantizer_with_layer_quantized(M, quantized: bool):
    ...
    # reset() owns the per-refit counters, so this helper does not have to be
    # updated every time one is added.
    quantizer.reset()
...
def test_finish_records_counters_on_success(nvfp4_module, monkeypatch):
    """finish() only validates; the counters it exposes drive the refit line."""
    M = nvfp4_module
    _fake_scaled_fp4_quant(monkeypatch, M)
    quantizer = _quantizer_with_layer_quantized(M, True)
    ...
    assert quantizer.quantized_events == 1
    assert quantizer.quantize_seconds > 0.0
...
def test_rollout_config_defaults(nvfp4_module):
    ...
    assert cfg.experimental_stacked_reload is False
